...
ad_headline: "合并和拆分 VSSM 文件 | C＃"
ad_description: "在 .NET 中高效地合并、拆分、移动、删除、交换、旋转和提取 VSSM 文件页面"

############################# Head ############################
head_title: "在 C# .NET 中合并和拆分 VSSM 文件并添加水印"
head_description: "C# .NET 文档合并库可将多个 VSSM 文件合并为一个或将单个 VSSM 文件拆分为多个文件。还可以从文档中移动、删除、旋转、交换和提取页面。"

############################# Header ############################
title: "在 .NET 中合并 VSSM 文件并添加水印"
description: "C# .NET 文档合并 API 将多个 VSSM 文件合并为一个文件，方法是将多个源文档中的选择数量的页面或一系列页面合并为一个文件。执行单个文档操作操作，例如移动、删除、旋转、交换和提取页面或将单个 VSSM 文档拆分为多个结果文档。"
...
content:
    enable: true
    block:
    - title_left: "在 C# 中合并 VSSM 文件并添加水印"
      content_left: |
          在 C# .NET 中加入 VSSM 文件，并将文本或图像水印添加到 .NET（C#、VB.NET、ASP.NET 和 .NET Core）应用程序中的单个结果文档。
    ...
    - title_left: "在 .NET 中拆分 VSSM 文件并添加水印"
      content_left: |
          将单个 VSSM 文档拆分为多个独立文档，并使用 C# .NET 将图像或文本水印插入到每个拆分的文件中。
