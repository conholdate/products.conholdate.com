...
content:
    enable: true
    block:
    - title_left: "So konvertieren Sie XLSX in XPS in Java"
      content_left: |
          Führen Sie die XLSX-zu-XPS-Dateikonvertierung in Java in drei einfachen Schritten durch. Sehen Sie sich das konvertierte Dokument so an, wie es ist, oder rendern Sie es so, dass es ohne externe Softwareabhängigkeit als HTML angezeigt wird.

          -   Erstellen Sie eine neue Instanz der **Converter**-Klasse und laden Sie die XLSX-Datei
          -   Legen Sie **ConvertOptions** für den XPS-Dokumenttyp fest
    ...
    - title_left: "Konvertieren Sie passwortgeschütztes XLSX in XPS"
      content_left: |
