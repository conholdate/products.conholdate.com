...
head_description: "Java TIF Signature API to add, edit, remove, verify and search digital signatures (text, image, metadata, QR-Code, stamp). View the signed TIF file in HTML."
...
content:
    enable: true
    block:
    - title_left: "Add Image Signatures to TIF in Java"
      content_left: |
          Insert custom image as an electronic signature to a TIF document in Java. Add company logo, stamp icon or name using different colors and text effects.
      ...
      title_right: "Inserting Digital Signatures to TIF"
      content_right: |
      ...
      code: |
          ```cs {linenos=false}
          Signature signature = new Signature("input.tif")
    ...
    - title_left: "Add Text Signatures to TIF in Java"
      content_left: |
          Add customized text signature to a TIF document using advanced text settings such as font color, size, name, text alignment and border adjustment.
