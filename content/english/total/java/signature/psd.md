...
head_description: "Java PSD Signature API to add, edit, remove, verify and search digital signatures (text, image, metadata, QR-Code, stamp). View the signed PSD file in HTML."
...
content:
    enable: true
    block:
    - title_left: "Add Image Signatures to PSD in Java"
      content_left: |
          Insert custom image as an electronic signature to a PSD document in Java. Add company logo, stamp icon or name using different colors and text effects.
      ...
      title_right: "Inserting Digital Signatures to PSD"
      content_right: |
      ...
      code: |
          ```cs {linenos=false}
          Signature signature = new Signature("input.psd")
    ...
    - title_left: "Add Text Signatures to PSD in Java"
      content_left: |
          Add customized text signature to a PSD document using advanced text settings such as font color, size, name, text alignment and border adjustment.
