...
head_description: "Java Presentation Signature API to add, edit, remove, verify and search digital signatures (text, image, metadata, QR-Code, stamp). View the signed Presentation file in HTML."
...
content:
    enable: true
    block:
    - title_left: "Add Image Signatures to Presentation in Java"
      content_left: |
          Insert custom image as an electronic signature to a Presentation document in Java. Add company logo, stamp icon or name using different colors and text effects.
      ...
      title_right: "Inserting Digital Signatures to Presentation"
      content_right: |
      ...
      code: |
          ```cs {linenos=false}
          Signature signature = new Signature("input.presentation")
          ...
          signature.sign("output.presentation", options);

          // Set options to view document as HTML
          HtmlViewOptions options = HtmlViewOptions.forEmbeddedResources("output{0}.html");

          // Instantiate Viewer with output file
          try (Viewer viewer = new Viewer("output.presentation")) {
            viewer.view(options);
            }
          ```
    - title_left: "Add Text Signatures to Presentation in Java"
      content_left: |
          Add customized text signature to a Presentation document using advanced text settings such as font color, size, name, text alignment and border adjustment.
