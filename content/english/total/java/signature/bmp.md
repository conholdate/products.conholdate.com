...
content:
    enable: true
    block:
    - title_left: "Add Image Signatures to BMP in Java"
      content_left: |
          Insert custom image as an electronic signature to a BMP document in Java. Add company logo, stamp icon or name using different colors and text effects.
      ...
      title_right: "Inserting Digital Signatures to BMP"
      content_right: |
      ...
      code: |
          ```cs {linenos=false}
          Signature signature = new Signature("input.bmp")
          ...
          signature.sign("output.bmp", options);
    ...
    - title_left: "Add Text Signatures to BMP in Java"
      content_left: |
          Add customized text signature to a BMP document using advanced text settings such as font color, size, name, text alignment and border adjustment.
