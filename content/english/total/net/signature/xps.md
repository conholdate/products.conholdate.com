...
head_description: "Digitally add signatures to an XML Paper Specification file (XPS) and 100+ other file formats in C# .NET. View the signed file in HTML using Conholdate.Total for .NET APIs."

############################# Header ############################
title: "Digitally Add Signatures to XPS Viewer"
description: "Digitally sign and verify signatures in an XML Paper Specification file (XPS) within your C#, ASP.NET, VB.NET & Xamarin applications. Implement Barcode, Text, Image, Metadata, QR Code, Form Field and stamp signatures in multiple forms by setting up customized text, font style, colors and adjusting advanced e-signatures properties in the document."
...
steps:
    enable: true
    title_left: "C# Code to Digitally Sign a XPS File"
    content_left: |
        [Conholdate.Total for .NET](https://products.conholdate.com/total/net/) supports signing an XML Paper Specification file (XPS) with digital signatures using a few lines of C# .NET code.
    ...
    code: |
        ```cs
        // Add digital e-signatures to XPS file using GroupDocs.Signature API
        // Instantiate Signature with input XPS document
        using (Signature signature = new Signature("input.xps"))
          {
          ...
          signature.Sign("output.xps", options);
