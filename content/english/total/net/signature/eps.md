...
############################# Head ############################
head_title: "Add Digital Signatures to EPS File Viewer in C#, VB.NET"
head_description: "Digitally add signatures to an encapsulated PostScript (EPS) file and 100+ other file formats in C# .NET. View the signed file in HTML using Conholdate.Total for .NET APIs."

############################# Header ############################
title: "Digitally Add Signatures to EPS Viewer"
description: "Digitally sign and verify signatures in an encapsulated PostScript (EPS) file within your C#, ASP.NET, VB.NET & Xamarin applications. Implement Barcode, Text, Image, Metadata, QR Code, Form Field and stamp signatures in multiple forms by setting up customized text, font style, colors and adjusting advanced e-signatures properties in the document."
...
steps:
    enable: true
    title_left: "C# Code to Digitally Sign a EPS File"
    content_left: |
        [Conholdate.Total for .NET](https://products.conholdate.com/total/net/) supports signing an encapsulated PostScript (EPS) file with digital signatures using a few lines of C# .NET code.
    ...
    code: |
        ```cs
        // Add digital e-signatures to EPS file using GroupDocs.Signature API
        // Instantiate Signature with input EPS document
        using (Signature signature = new Signature("input.eps"))
          {
          ...
          signature.Sign("output.eps", options);
