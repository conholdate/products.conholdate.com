...
############################# Head ############################
head_title: "Add Digital Signatures to TEX File Viewer in C#, VB.NET"
head_description: "Digitally add signatures to a source document prepared by LaTeX (TEX) and 100+ other file formats in C# .NET. View the signed file in HTML using Conholdate.Total for .NET APIs."

############################# Header ############################
title: "Digitally Add Signatures to TEX Viewer"
description: "Digitally sign and verify signatures in a source document prepared by LaTeX (TEX) within your C#, ASP.NET, VB.NET & Xamarin applications. Implement Barcode, Text, Image, Metadata, QR Code, Form Field and stamp signatures in multiple forms by setting up customized text, font style, colors and adjusting advanced e-signatures properties in the document."
...
steps:
    enable: true
    title_left: "C# Code to Digitally Sign a TEX File"
    content_left: |
        [Conholdate.Total for .NET](https://products.conholdate.com/total/net/) supports signing a source document prepared by LaTeX (TEX) with digital signatures using a few lines of C# .NET code.
    ...
    code: |
        ```cs
        // Add digital e-signatures to TEX file using GroupDocs.Signature API
        // Instantiate Signature with input TEX document
        using (Signature signature = new Signature("input.tex"))
          {
          ...
          signature.Sign("output.tex", options);
