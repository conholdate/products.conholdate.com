...
content:
    enable: true
    block:
    - title_left: "כיצד להמיר RTF ל-XLT ב-Java"
      content_left: |
          בצע המרת קבצי RTF ל-XLT ב-Java באמצעות שלושה שלבים פשוטים. הצג את מסמך ה-MHTML שהומר כפי שהוא או רנדר והצג אותו כ-HTML ללא שימוש בתוכנה חיצונית כלשהי.
