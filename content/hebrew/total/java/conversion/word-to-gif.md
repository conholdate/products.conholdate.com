...
content:
    enable: true
    block:
    - title_left: "כיצד להמיר WORD ל-GIF ב-Java"
      content_left: |
          בצע המרת קבצי WORD ל-GIF ב-Java באמצעות שלושה שלבים פשוטים. הצג את מסמך ה-MHTML שהומר כפי שהוא או רנדר והצג אותו כ-HTML ללא שימוש בתוכנה חיצונית כלשהי.

          -   צור מופע חדש של מחלקה **Converter** וטען את קובץ ה-WORD
          -   הגדר את **ConvertOptions** עבור סוג המסמך GIF
          -   התקשר לשיטת **Convert** של מופע המחלקה **Converter** להמרה ל-GIF
          -   הגדר אפשרויות עבור מציג HTML
          -   צור אובייקט **Viewer** כדי להציג GIF שהומר כ-HTML
