...
content:
    enable: true
    block:
    - title_left: "כיצד להמיר DOCM ל-WEB ב-Java"
      content_left: |
          בצע המרת קבצי DOCM ל-WEB ב-Java באמצעות שלושה שלבים פשוטים. הצג את מסמך ה-MHTML שהומר כפי שהוא או רנדר והצג אותו כ-HTML ללא שימוש בתוכנה חיצונית כלשהי.

          -   צור מופע חדש של מחלקה **Converter** וטען את קובץ ה-DOCM
          -   הגדר את **ConvertOptions** עבור סוג המסמך WEB
          -   התקשר לשיטת **Convert** של מופע המחלקה **Converter** להמרה ל-WEB
          -   הגדר אפשרויות עבור מציג HTML
          -   צור אובייקט **Viewer** כדי להציג WEB שהומר כ-HTML
