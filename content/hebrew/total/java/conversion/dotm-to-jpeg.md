...
content:
    enable: true
    block:
    - title_left: "כיצד להמיר DOTM ל-JPEG ב-Java"
      content_left: |
          בצע המרת קבצי DOTM ל-JPEG ב-Java באמצעות שלושה שלבים פשוטים. הצג את מסמך ה-MHTML שהומר כפי שהוא או רנדר והצג אותו כ-HTML ללא שימוש בתוכנה חיצונית כלשהי.

          -   צור מופע חדש של מחלקה **Converter** וטען את קובץ ה-DOTM
          -   הגדר את **ConvertOptions** עבור סוג המסמך JPEG
          -   התקשר לשיטת **Convert** של מופע המחלקה **Converter** להמרה ל-JPEG
          -   הגדר אפשרויות עבור מציג HTML
          -   צור אובייקט **Viewer** כדי להציג JPEG שהומר כ-HTML
