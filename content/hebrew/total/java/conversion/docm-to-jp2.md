...
content:
    enable: true
    block:
    - title_left: "כיצד להמיר DOCM ל-JP2 ב-Java"
      content_left: |
          בצע המרת קבצי DOCM ל-JP2 ב-Java באמצעות שלושה שלבים פשוטים. הצג את מסמך ה-MHTML שהומר כפי שהוא או רנדר והצג אותו כ-HTML ללא שימוש בתוכנה חיצונית כלשהי.

          -   צור מופע חדש של מחלקה **Converter** וטען את קובץ ה-DOCM
          -   הגדר את **ConvertOptions** עבור סוג המסמך JP2
          -   התקשר לשיטת **Convert** של מופע המחלקה **Converter** להמרה ל-JP2
          -   הגדר אפשרויות עבור מציג HTML
          -   צור אובייקט **Viewer** כדי להציג JP2 שהומר כ-HTML
