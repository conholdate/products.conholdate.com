...
content:
    enable: true
    block:
    - title_left: "כיצד להמיר WORD ל-TEX ב-Java"
      content_left: |
          בצע המרת קבצי WORD ל-TEX ב-Java באמצעות שלושה שלבים פשוטים. הצג את מסמך ה-MHTML שהומר כפי שהוא או רנדר והצג אותו כ-HTML ללא שימוש בתוכנה חיצונית כלשהי.

          -   צור מופע חדש של מחלקה **Converter** וטען את קובץ ה-WORD
          -   הגדר את **ConvertOptions** עבור סוג המסמך TEX
          -   התקשר לשיטת **Convert** של מופע המחלקה **Converter** להמרה ל-TEX
          -   הגדר אפשרויות עבור מציג HTML
          -   צור אובייקט **Viewer** כדי להציג TEX שהומר כ-HTML
