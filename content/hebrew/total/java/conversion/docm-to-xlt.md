...
content:
    enable: true
    block:
    - title_left: "כיצד להמיר DOCM ל-XLT ב-Java"
      content_left: |
          בצע המרת קבצי DOCM ל-XLT ב-Java באמצעות שלושה שלבים פשוטים. הצג את מסמך ה-MHTML שהומר כפי שהוא או רנדר והצג אותו כ-HTML ללא שימוש בתוכנה חיצונית כלשהי.

          -   צור מופע חדש של מחלקה **Converter** וטען את קובץ ה-DOCM
          -   הגדר את **ConvertOptions** עבור סוג המסמך XLT
          -   התקשר לשיטת **Convert** של מופע המחלקה **Converter** להמרה ל-XLT
          -   הגדר אפשרויות עבור מציג HTML
          -   צור אובייקט **Viewer** כדי להציג XLT שהומר כ-HTML
