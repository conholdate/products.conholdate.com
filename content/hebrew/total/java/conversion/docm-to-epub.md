...
content:
    enable: true
    block:
    - title_left: "כיצד להמיר DOCM ל-EPUB ב-Java"
      content_left: |
          בצע המרת קבצי DOCM ל-EPUB ב-Java באמצעות שלושה שלבים פשוטים. הצג את מסמך ה-MHTML שהומר כפי שהוא או רנדר והצג אותו כ-HTML ללא שימוש בתוכנה חיצונית כלשהי.
