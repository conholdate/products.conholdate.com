...
content:
    enable: true
    block:
    - title_left: "JavaでOTSをICOに変換する方法"
      content_left: |
          3つの簡単な手順を使用して、JavaでOTSからICOファイルへの変換を実行します。変換されたドキュメントをそのまま表示するか、外部のソフトウェアに依存せずにHTMLとして表示するようにレンダリングします。
    ...
    - title_left: "パスワードで保護されたOTSをICOに変換する"
      content_left: |
