...
content:
    enable: true
    block:
    - title_left: "JavaでTSVをOTPに変換する方法"
      content_left: |
          3つの簡単な手順を使用して、JavaでTSVからOTPファイルへの変換を実行します。変換されたドキュメントをそのまま表示するか、外部のソフトウェアに依存せずにHTMLとして表示するようにレンダリングします。
    ...
    - title_left: "パスワードで保護されたTSVをOTPに変換する"
      content_left: |
